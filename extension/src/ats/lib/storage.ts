 ...
 * All ATS settings are stored under the 'settings.ats' namespace
 */
...
/**
 * Get ATS settings from storage
 */
export async function getAtsSettings(): Promise<AtsSettings> {
  const result = await chrome.storage.local.get(['settings']);
  return result.settings?.ats || {
    enabled: false,
    model: 'gpt-4o-mini',
    language: 'en',
...
/**
 * Save ATS settings to storage
 */
export async function saveAtsSettings(atsSettings: Partial<AtsSettings>): Promise<void> {
  const result = await chrome.storage.local.get(['settings']);
  const current = result.settings || {};
  await chrome.storage.local.set({
    settings: {
      ...current,
      ats: { ...(current.ats || {}), ...atsSettings },
    },
  });
}
...
export async function getProfile(): Promise<Profile | null> {
  const result = await chrome.storage.local.get(['settings']);
  return result.settings?.ats?.profile || null;
}
...
export async function getTrackedJobs(): Promise<TrackedJob[]> {
  const result = await chrome.storage.local.get(['settings']);
  return result.settings?.ats?.trackedJobs || [];
}
...
export async function saveTrackedJobs(jobs: TrackedJob[]): Promise<void> {
  ...
  await chrome.storage.local.set({
    settings: {
      ...current,
      ats: { ...(current.ats || {}), trackedJobs: jobs },
    },
  });
}
...
export async function getCoverLetterTemplates(): Promise<CoverLetterTemplate[]> {
  const result = await chrome.storage.local.get(['settings']);
  return result.settings?.ats?.coverLetterTemplates || getDefaultCoverLetterTemplates();
}
