...
export const ATSOptimizations: React.FC<ATSOptimizationsProps> = ({
  optimizations,
  onChange,
  language,
  onOptimizationFocus,
  ...
  const toggleOptimization = (id: string) => {
    onChange(optimizations.map((opt) => (opt.id === id ? { ...opt, applied: !opt.applied } : opt)));
  };
  ...
  return (
    <div className="section">
      <h2 className="section-title">✨ {t(language, 'opt.section')}</h2>

      {optimizations.length === 0 ? (
        <div className="alert alert-info">{t(language, 'opt.clickOptimize')}</div>
      ) : (
        <>
          <div className="alert alert-success optimization-summary">
            {appliedOptimizations.length}{' '}
            {appliedOptimizations.length !== 1
              ? t(language, 'opt.optimizations')
              : t(language, 'opt.optimization')}{' '}
            {t(language, 'opt.applied')}.{t(language, 'opt.hoverTip')}
          </div>
          ...
          <div className="optimization-details-section">
            <h3 className="subsection-title">{t(language, 'opt.detailsTitle')}</h3>
            ...
            {optimizations.map((opt) => (
              <div key={opt.id} className="optimization-card">
                ...
                <div className="optimization-change">{opt.change}</div>

                <div className="text-comparison">
                  <div>
                    <div className="text-label">{t(language, 'opt.original')}</div>
                    <div className="text-box text-original">{opt.originalText}</div>
                  </div>
                  ...
                  <div>
                    ...
                    <div
                      ...
                      onClick={() =>
                        onOptimizationFocus(focusedOptimizationId === opt.id ? null : opt.id)
                      }
                      style={{ cursor: 'pointer' }}
                      title={
                        opt.applied
                          ? t(language, 'opt.clickToHighlight')
                          : t(language, 'opt.applyFirstToHighlight')
                      }
                    >
                      {opt.optimizedText}
                    </div>
